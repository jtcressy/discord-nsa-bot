...
description = '''NSA Bot - Every key you press, every like and share, I'll be watching you.
Commands currently in use on this bot:'''
...
def get_dbclient():
    logger = get_logger()
    try:
        mongodb_url = environ['NSABOT_MONGODB_URL']
    except KeyError as e:
        logger.error(f"Unable to setup database, missing MongoDB URL. Use env var 'NSA_BOT_MONGODB_URL' with the format 'mongodb://username:password@hostname:27017/database'.")
        return None
...
def run(token=""):
    logger = logger_setup()
    for package in package_contents(nsabot.cogs):
        try:
